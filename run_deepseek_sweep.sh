...
run_exact () {
  ...
  CUDA_VISIBLE_DEVICES=0,1,2,3 torchrun --nproc_per_node=4 finetune_exact_masks.py \
    ...
    --amp_dtype bf16 \
    --save_mode sharded \
    --save_path "${OUT_DIR}/finetuned_exact_deepseek_${TASK}_${STEPS}steps_experts_train_mode_mass${MASS}"
}
...
run_uniform () {
  ...
  CUDA_VISIBLE_DEVICES=0,1,2,3 torchrun --nproc_per_node=4 finetune_exact_masks.py \
    ...
    --save_path "${OUT_DIR}/finetuned_uniform_deepseek_${TASK}_${STEPS}steps_experts_train_mode_mass0.9"
}
