...
"""
EXACT-Compressed Qwen2-MoE Benchmark
Tests EXACT adaptive compression on Qwen2-MoE
"""
...
from qwen2_moe.modeling_qwen2_moe import Qwen2MoeSparseMoeBlock, Qwen2MoeDecoderLayer, Qwen2MoeModel
from qwen2_moe.configuration_qwen2_moe import Qwen2MoeConfig
from qwen2_moe.modeling_qwen2_moe_exact import qwen2_moe_to_exact
...
parser.add_argument('--profiling_json', type=str, default='artifacts/exact_profiling/qwen1.5-moe-a2.7b_real_e60_layer.json',
...
def qwen2_moe_mlp_run():
    ...
    dense_model = Qwen2MoeSparseMoeBlock(configuration)
    exact_model = qwen2_moe_to_exact(dense_model, args.profiling_json)
    ...
    if args.time:
        ...
        print("qwen2_moe,mlp,EXACT,%d,%d,%d,%d,%d,%d,%s,%s" %
    ...
    if args.profile:
        prof = torch.profiler.profile(
            ...
            on_trace_ready=torch.profiler.tensorboard_trace_handler(
                './outputs/profiler/Qwen2MoE_EXACT_MLP_'+torch.cuda.get_device_name().split(' ')[1].split('-')[0]+'_'+str(args.batch_size)),
            record_shapes=True,
            profile_memory=True,
            with_stack=True
        )
...
def qwen2_moe_layer_run():
    ...
    layer = Qwen2MoeDecoderLayer(configuration, 0)
    
    # Convert the MoE block to EXACT
    layer.mlp = qwen2_moe_to_exact(layer.mlp, args.profiling_json)
    ...
    if args.time:
        ...
        print("qwen2_moe,layer,EXACT,%d,%d,%d,%d,%d,%d,%s,%s" %
              (ITER, args.batch_size, args.seq_len, args.hidden_size, args.intermediate_size,
               args.experts, (end - start)*1000, configuration._attn_implementation))
    ...
    if args.profile:
        prof = torch.profiler.profile(
            ...
            schedule=torch.profiler.schedule(wait=1, warmup=10, active=10, repeat=1),
            on_trace_ready=torch.profiler.tensorboard_trace_handler(
                './outputs/profiler/Qwen2MoE_EXACT_Layer_'+torch.cuda.get_device_name().split(' ')[1].split('-')[0]+'_'+str(args.batch_size)),
            record_shapes=True,
            profile_memory=True,
            with_stack=True
        )
...
def qwen2_moe_model_run():
    """Test EXACT-compressed full 28-layer model"""
    # print("[!] Model mode: Converting all MoE blocks in 28-layer model to EXACT")
    
    configuration.num_hidden_layers = 28
    model = Qwen2MoeModel(configuration)
    
    # Convert all MoE blocks to EXACT (every decoder_sparse_step layers)
    for layer_idx, layer in enumerate(model.layers):
        if hasattr(layer, 'mlp') and type(layer.mlp).__name__ == 'Qwen2MoeSparseMoeBlock':
            layer.mlp = qwen2_moe_to_exact(layer.mlp, args.profiling_json)
    ...
    if args.time:
        ...
        print("qwen2-moe,model,EXACT,%d,%d,%d,%d,%d,%d,%s,%s" %
              (ITER, args.batch_size, args.seq_len, args.hidden_size, args.intermediate_size,
               args.experts, (end - start)*1000, configuration._attn_implementation))
    ...
    if args.profile:
        prof = torch.profiler.profile(
            ...
            on_trace_ready=torch.profiler.tensorboard_trace_handler(
                './outputs/profiler/Qwen2MoE_EXACT_model_'+torch.cuda.get_device_name().split(' ')[1].split('-')[0]),
            record_shapes=True,
            profile_memory=True,
            with_stack=True
        )
