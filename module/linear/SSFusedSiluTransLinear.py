...
def sparse_sparse_mul_fused_silu_trans_dispatch(A_sparse_values, A_sparse_indices, A_sparse_metadata, B_values, B_indices, m, k, n, structure_N=None, structure_M=None):
    # Use provided N, M or fall back to global defaults
    if structure_N is None:
        structure_N = N
    if structure_M is None:
        structure_M = M
    # print(f"[sparse_sparse_mul_fused_silu_trans_dispatch] N={structure_N}, M={structure_M}")    
    output = samoyeds_kernel.ssmm_fused_silu_trans(A_sparse_values,  # lhs_matrix, compressed values
...
class SSFusedSiluTransLinear(nn.Module):
    def __init__(self, original = None, in_features = 1024, out_features = 4096, skip_sparsifier=False):
        ...
        if original is None:
            ...
            self.original_out_features = original.out_features
    ...
    def forward(self, input, idx):
        # print('SSFusedSiluTransLinear forward: weight:', self.weight.shape, ', input:', input.shape, ', idx:', idx.shape)
        # new_idx = torch.arange(int(input.shape[0]), device=input.device)
        # idx = torch.arange(int(input.shape[0]), device=input.device)
        # print(f"[EXACT] SSFusedSiluTransLinear forward path: weight={self.weight.shape}, N={self.N}, M={self.M}")
        new_idx = padding_idx(idx, 64)
        output = sparse_sparse_mul_fused_silu_trans_dispatch(   self.weight, self.indices, self.metadata, input, new_idx,
                                                                self.original_out_features,
                                                                self.original_in_features, input.shape[0],
                                                                self.N, self.M)
        # print('SSFusedSiluTransLinear output: ', output[:, 0:idx.shape[0]].shape)
        return output
