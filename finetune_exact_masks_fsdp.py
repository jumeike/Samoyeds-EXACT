...
def build_param_masks(model, masks, name_prefix=""):
    """
    Build stable parameter-reference masks for expert weights.
    This avoids brittle FSDP name matching at save time.
    """
    if masks is None:
        return []
    base = _unwrap_model(model)
    param_masks = []
    for l_idx, layer in enumerate(base.model.layers):
        experts_path, experts = _get_layer_expert_path_and_container(layer)
        if experts is None:
            continue
        for e_idx, expert in _iter_experts(experts):
            ...
            for name in proj_names:
                linear = getattr(expert, name)
                key = f"{name_prefix}model.layers.{l_idx}.{experts_path}.{e_idx}.{name}.weight"
                if key in masks:
                    param_masks.append((linear.weight, masks[key].detach().cpu()))
    return param_masks
...
def apply_masks(model, masks, name_prefix="", param_masks=None):
    if masks is None:
        return 0, 0
    if param_masks:
        matched = 0
        for param, mask_cpu in param_masks:
            mask = mask_cpu.to(param.device, dtype=param.dtype)
            param.data.mul_(mask)
            matched += 1
            if torch.isnan(param.data).any() or torch.isinf(param.data).any():
                print("WARNING: NaN/Inf detected after applying param-ref mask")
        return matched, len(param_masks)
...
def main():
    ...
    parser.add_argument('--strict_sharded_presave_mask', action='store_true',
                        help='For save_mode=sharded, reapply masks under summon_full_params before saving (more reliable, higher RAM)')
    ...
    if use_fsdp:
        ...
        if args.load_sharded_dir and args.local_mask_on_sharded:
            ...
            with FSDP.summon_full_params(model):
                ...
                param_masks = build_param_masks(model, masks)
                matched, total = apply_masks(model, masks, param_masks=param_masks)
                if rank == 0:
                    print(f"[mask] initial apply matched {matched}/{total}")
    elif use_ddp:
        ...
        if (step + 1) % args.grad_accum == 0:
            ...
            with torch.no_grad():
                if use_fsdp:
                    ...
                    if args.mask_every > 0 and (step + 1) // args.grad_accum % args.mask_every == 0:
                        apply_masks(model, masks, param_masks=param_masks)
    ...
    if args.save_mode != 'none':
        ...
        if use_fsdp:
            if args.save_mode == 'full':
                ...
                if masks is not None:
                    with torch.no_grad():
                        if args.strict_sharded_presave_mask:
                            with FSDP.summon_full_params(model, offload_to_cpu=True):
                                # Refresh param refs inside summon context.
                                param_masks = build_param_masks(model, masks)
                                matched, total = apply_masks(model, masks, param_masks=param_masks)
                        else:
                            matched, total = apply_masks(model, masks, param_masks=param_masks)
